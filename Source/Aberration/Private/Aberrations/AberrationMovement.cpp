...
void AAberrationMovement::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

	//BUG: tick is not ticking
	LOG("Tick");
	...
	const float StartLocation = !GoingForward ? FinalXPosition : StartXPosition;
	...
	if (NearlyEqual || OffLimits)
	{
		...
		Mesh->SetRelativeLocation(FVector(Location.X, StartLocation, Location.Z));
	}
	
	const float XOffsetToAdd = Speed * DeltaTime;
	
	const FVector LocationToAdd = FVector(0, XOffsetToAdd, 0);
